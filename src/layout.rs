...
impl<'a> LayoutBox<'a> {
    pub fn new(box_type: BoxType, styled_node: &'a StyleNode<'a>) -> LayoutBox<'a> {
        LayoutBox{
            box_type,
            styled_node,
    ...
    fn calculate_inline_position(&mut self, b_box: Dimensions){
        ...
        d.content.y = b_box.content.height + b_box.current.y + d.margin.top + d.border.top + d.padding.top;
    }
    ...
    fn calculate_width(&mut self, b_box: Dimensions){
        ...
        let width: f32 = get_absolute_num(style, b_box, "width").unwrap_or(0.0);
    ...
    fn calculate_position(&mut self, b_box: Dimensions){
        ...
        d.content.x = b_box.content.x + b_box.margin.left + d.border.left + d.padding.left;
...
fn get_absolute_num(s_node: &StyleNode, b_box: Dimensions, prop: &str) -> Option<f32>{
...
fn build_layout_tree<'a> (node: &'a StyleNode) -> LayoutBox<'a>{
